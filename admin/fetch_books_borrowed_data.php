...
$sql = "SELECT b.borrowed_date, COUNT(*) AS count
        FROM borrowings AS b
        LEFT JOIN members AS m ON b.member_id = m.member_id
        WHERE b.borrowed_date IS NOT NULL AND b.status = 'Borrowed'";
...
if ($departmentFilter || $yearFilter || $memberTypeFilter || $timeFilter) {
    ...
    if ($timeFilter === 'weekly') {
        ...
        if ($startDate && $endDate) {
            $sql .= " AND b.borrowed_date BETWEEN '$startDate' AND '$endDate'";
        }
...
while ($row = $result->fetch_assoc()) {
    $response['labels'][] = $row['borrowed_date'];
